...
(defn write-to-kafka [ads kafka-host]
  ;; Put some crap in Kafka
  (println "Setting up kafka topic.")
  (with-open [p (producer {"bootstrap.servers" kafka-host}
...
(defn run [throughput with-skew? kafka-host redis-host]
  ...
  (let [{campaigns :campaigns ads :ads} (load-ids)]
    (if (or (nil? campaigns) (nil? ads))
      ...
      (let [campaigns (make-ids num-campaigns)
        ...
        (write-to-kafka ads (conf :kafka-host))
        (write-ids campaigns ads))
      (write-to-redis campaigns ads))))

(defn get-conf []
      (let [redis-host (System/getenv "REDIS_HOST")
            kafka-port (System/getenv "KAFKA_PORT")
            kafka-broker (System/getenv "KAFKA_BROKER")
            workload (System/getenv "LOAD")
            skew-enabled (System/getenv "SKEW_ENBALED")
            kafka-host (str kafka-broker ":" kafka-port)]
           {:redis-host redis-host
            :kafka-broker kafka-broker
            :kafka-port kafka-port
            :workload workload
            :skew-enabled skew-enabled
...
(defn -main
  ...
  (let [{:keys [options arguments errors summary]} (cli/parse-opts args cli-options)
        conf (get-conf)
        kafka-host (get conf :kafka-host)
    ...
    (cond
      ...
      (:run options)                          (run (:throughput options) (:with-skew options) kafka-host redis-host)
